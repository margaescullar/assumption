    ...
    <div class="box-body form-horizontal">
        <form id="myForm" method="post" target='_blank' action="">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <br>
                    <a href='javascript:void(0)' class='btn btn-primary col-sm-12' onclick='generate_report(level.value, school_year.value, period.value)'>Generate Report</button></a>
                </div>
            </div>
            <div class="form-group">
                <div class="col-sm-3">
...
<script>
    ...
    $("#level").on('change', function (e) {
        if ($("#level").val() == "Grade 11" || $("#level").val() == "Grade 12" || $("#level").val() == "1st Year" || $("#level").val() == "2nd Year" || $("#level").val() == "3rd Year" || $("#level").val() == "4th Year") {
    ...
    function toPDF() {
        document.getElementById("myForm").action = "{{url('/accounting/payment_plans_student_pdf')}}";
    }

    function toEXCEL() {
        document.getElementById("myForm").action = "{{url('/accounting/payment_plans_student_excel')}}";
    }

    function generate_report(level, school_year, period) {
        var array = {};
        array['level'] = level;
        ...
        $.ajax({
            type: "GET",
            url: "/accounting/ajax/get_paymentplans_student",
            data: array,
